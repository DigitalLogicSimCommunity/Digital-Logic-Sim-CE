...
public static class ChipLoader {
  ...
  public static SavedChip[] GetAllSavedChips(string[] chipPaths) {
    ...
    for (int i = 0; i < chipPaths.Length; i++) {
      ...
      if (!chipSaveString.Contains("wireType") || chipSaveString.Contains(
              "outputPinNames") || !chipSaveString.Contains("outputPins")) {
  ...
  public static async void LoadAllChips(string[] chipPaths, Manager manager) {
    ...
    ProgressBar progressBar =
        ProgressBar.New("Loading All Chips...", wholeNumbers: true);
    ...
    for (int i = 0; i < savedChips.Length; i++) {
      ...
      progressBar.SetValue(i + manager.builtinChips.Length,
                           "Loading '" + chipToTryLoad.name + "'...");
      try {
        ChipSaveData loadedChipData =
            LoadChip(chipToTryLoad, loadedChips, manager.wirePrefab);
        Chip loadedChip = manager.LoadChip(loadedChipData);
        loadedChips.Add(loadedChip.chipName, loadedChip);
      } catch (Exception e) {
        DLSLogger.LogWarning("Custom Chip '" + chipToTryLoad.name +
                                 "' could not be loaded!",
                             e.ToString());
      }
      await Task.Yield();
