...
public class ChipEditorOptions : MonoBehaviour {
	...
	public void SetUIValues(ChipEditor editor) {
		...
		showZoomHelperToggle.SetIsOnWithoutNotify(PlayerPrefs.GetInt("ShowZoomHelper", 1) == 1);
		mouseWheelSensitivitySlider.SetValueWithoutNotify(PlayerPrefs.GetFloat("MouseSensitivity", 0.1f));
		camMoveSpeedSlider.SetValueWithoutNotify(PlayerPrefs.GetFloat("CamMoveSpeed", 12f));
	...
	public void OnMouseWheelSensitivityChanged() {
		ZoomManager.instance.mouseWheelSensitivity = mouseWheelSensitivitySlider.value;
