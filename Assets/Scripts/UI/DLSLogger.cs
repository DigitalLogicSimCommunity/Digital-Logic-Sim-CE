...
public class DLSLogger : MonoBehaviour {
    ...
    void Awake() {
        ...
        debugMessageTemplate = Instantiate(logMsgTemplate, transform, false).GetComponent<LoggingMessage>();
        debugMessageTemplate.gameObject.SetActive(false);

        warningMessageTemplate = Instantiate(logMsgTemplate, transform, false).GetComponent<LoggingMessage>();
        ...
        errorMessageTemplate = Instantiate(logMsgTemplate, transform, false).GetComponent<LoggingMessage>();
    ...
    static GameObject NewLogMessage(LoggingMessage template, string message, string details) {
        ...
        GameObject newMessage = Instantiate(template.gameObject, instance.loggingMsgsHolder, false);
    ...
    static void UpdateOpenLogsButton() {
        if (showDebug && allWarnLogs.Count == 0 && allErrorLogs.Count == 0) {
            ...
            instance.logsCounterText.text = allDebugLogs.Count < 100 ? allDebugLogs.Count.ToString() : "99+";
        } else if (showWarn && allErrorLogs.Count == 0) {
            instance.openLogsButton.image.color = allWarnLogs.Count > 0 ? instance.warnCol : instance.debugCol;
            instance.openLogsButton.image.sprite = instance.warnSprite;
            instance.logsCounterText.text = allWarnLogs.Count < 100 ? allWarnLogs.Count.ToString() : "99+";
        } else {
            instance.openLogsButton.image.color = allErrorLogs.Count > 0 ? instance.errorCol : instance.debugCol;
            instance.openLogsButton.image.sprite = instance.errorSprite;
            instance.logsCounterText.text = allErrorLogs.Count < 100 ? allErrorLogs.Count.ToString() : "99+";
        }
    }

    public static void Log(string message, string details = "") {
        Debug.Log(!String.IsNullOrEmpty(details) ? message + ": " + details : message);
        GameObject newMessage = NewLogMessage(debugMessageTemplate, message, details);
    ...
    public static void LogWarning(string message, string details = "") {
        Debug.LogWarning(!String.IsNullOrEmpty(details) ? message + ": " + details : message);
        GameObject newMessage = NewLogMessage(warningMessageTemplate, message, details);
    ...
    public static void LogError(string message, string details = "") {
        Debug.LogError(!String.IsNullOrEmpty(details) ? message + ": " + details : message);
        GameObject newMessage = NewLogMessage(errorMessageTemplate, message, details);
