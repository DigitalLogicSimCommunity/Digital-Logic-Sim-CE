...
public class ChipBarUI : MonoBehaviour {
	...
	public static Dictionary<string, int> folders = new Dictionary<string, int>() {
		{"Basic", 0}, {"Advanced", 1}, {"User", 2}
	};
	...
	void Awake () {
		...
		selectedFolderDropdown.AddOptions(new List <TMP_Dropdown.OptionData> {newFolderOption});
	...
	public void SelectFolder() {
		if (selectedFolderDropdown.value == selectedFolderDropdown.options.Count - 1) {
	...
	public void CheckFolderName(bool endEdit = false) {
		...
		SubmitNewFolder.interactable = validName.Length > 0 && FolderNameAvailable(validName);
