...
public class ScalingManager : MonoBehaviour {
    ...
    void Update() {
        propertiesUIXZoom = Mathf.Lerp(minPropertiesUIXZoom, maxPropertiesUIXZoom, ZoomManager.zoom);
        propertiesUIX = Mathf.Lerp(minPropertiesUIX, maxPropertiesUIX, scale) - propertiesUIXZoom;
    }
    ...
    static void CalcValues() {
        ...
        chipInteractionBoundsBorder = Mathf.Lerp(minChipInteractionBoundsBorder, maxChipInteractionBoundsBorder, scale);
        chipStackSpace = Mathf.Lerp(minChipStackSpace, maxChipStackSpace, scale);
        pinDisplayPadding = Mathf.Lerp(minPinDisplayPadding, maxPinDisplayPadding, scale);
        pinDisplayTextOffset = Mathf.Lerp(minPinDisplayTextOffset, maxPinDisplayTextOffset, scale);
        ioBarDistance = Mathf.Lerp(minIOBarDistance, maxIOBarDistance, scale);
        ioBarGraphicWidth = Mathf.Lerp(minIOBarGraphicWidth, maxIOBarGraphicWidth, scale);
        groupSpacing = Mathf.Lerp(minGroupSpacing, maxGroupSpacing, scale);

        pinDisplayFontSize = Mathf.Clamp(fontSize, minPinDisplayFontSize, maxPinDisplayFontSize);
        packageFontSize = Mathf.Clamp(fontSize * 1.5f, minPackageFontSize, maxPackageFontSize);
        wireThickness = Mathf.Clamp(pinSize * 1.5f, minWireThickness, maxWireThickness);
